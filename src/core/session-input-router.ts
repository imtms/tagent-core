...
const criteria = (summary: string, intent: SessionInputIntent) => intent === "discussion"
  ? [`直接回答“${summary}”`, "明确区分已知事实、推断和不确定性"]
  : [`完成目标：${summary}`, "提供可验证的结果或明确说明阻塞原因"];
...
const DISCUSSION = /^(为什么|为何|怎么理解|解释一下|你觉得|是否应该|what|why|how|explain|compare)\b|[?？]$/i;
...
export class SessionInputRouter {
  analyze(content: string, activeRun?: TaskRun): SessionInputAnalysis {
    ...
    let intent: SessionInputIntent = DISCUSSION.test(source) ? "discussion" : "new_task";
    let relation: SessionInputAnalysis["relation"] = "independent";
    let targetRunId: string | null = null;
    let priority = urgency === "critical" ? 950 : intent === "discussion" ? 350 : 500;
    let confidence = 0.82;
    let reason = activeRun ? "Input does not safely match an active-run control rule; keep it as separate queued work." : "No active TaskRun; create a concise TaskRun contract.";
