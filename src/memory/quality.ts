...
export function isDurableMemory(record:WarmMemory){const title=record.kind==="preference"?record.dimension:record.title;const content=record.kind==="preference"?record.value:record.content;const combined=`${title}\n${content}\n${record.summary}`;if([title,content,record.summary].some(isControlPlaneText)||hasMalformedNegation(combined))return false;if(record.provenance?.sourceRole==="system"||record.provenance?.evidenceClass==="task_outcome")return false;if(isOneOffRequest(content)&&record.kind!=="preference"&&record.kind!=="procedure")return false;if(record.kind==="episode"&&/(?:已发布|已完成|验证|审计|检查|测试|部署|合并|同步|TaskRun)/i.test(combined))return false;return semanticConsistent(record);}
...
export function conflictKey(card:MemoryCard){const s=card.semantic;return s?`${normalizeTerm(s.subject)}|${normalizePredicate(s.predicate)}|${normalizeTerm(s.object)}`:`${inferMemoryDomain(card)}:${card.title.toLowerCase().replace(/(?:不|未|无|仍|当前|目前|fact:|episode:|\s)/g,"")}`;}
