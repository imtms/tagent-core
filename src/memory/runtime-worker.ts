...
export class LocalMemoryWorker {
  private captureRunning=false; private maintenanceRunning=false; private captureTimer?:ReturnType<typeof setInterval>; private maintenanceTimer?:ReturnType<typeof setInterval>;
  ...
  async captureTick(){if(this.captureRunning)return false;this.captureRunning=true;try{let count=0;while(count<20&&await this.capture.runOnce())count++;return count>0;}finally{this.captureRunning=false;}}
  async maintenanceTick(){if(this.maintenanceRunning)return false;this.maintenanceRunning=true;try{await this.lifecycle.promote(this.access);const candidates=await this.lifecycle.topicCandidates(this.access);for(const topic of candidates.slice(0,20))await this.consolidator.consolidate(this.access,topic.topicId);await this.reconciler.verify(this.access,candidates.map((x)=>x.topicId));await this.reconciler.cleanupStaged();return true;}finally{this.maintenanceRunning=false;}}
}
