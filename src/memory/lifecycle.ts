...
export class MemoryLifecycle {
  ...
  async integrate(access:AccessContext,proposal:ExtractionProposal){
    ...
    for(const rawCandidate of proposal.records){
      if(!isDurableMemory(rawCandidate))continue;
      const canonicalTopic=canonicalizeTopic(rawCandidate);
      const routedCandidate=canonicalTopic?{...rawCandidate,topicIds:[canonicalTopic]}:rawCandidate;
      const candidate=routedCandidate.tier==="hot"&&!routedCandidate.expiresAt&&this.options.hotTtlMs?{...routedCandidate,expiresAt:Date.now()+this.options.hotTtlMs}:routedCandidate;
