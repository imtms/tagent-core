...
import { canonicalFingerprint, canonicalizeTopic, isDurableMemory } from "./quality.js";
...
export class MemoryLifecycle {
  ...
  async integrate(access:AccessContext,proposal:ExtractionProposal){
    ...
    for(const raw of proposal.records){
      ...
      if(conflict){const oldLifecycle=state(conflict),superseded:WarmMemory=conflict.kind==="preference"?{...conflict,status:"superseded",lifecycle:{...oldLifecycle,lastSeenAt:now,staleAt:now},updatedAt:now}:{...conflict,status:"superseded",validTo:now,lifecycle:{...oldLifecycle,lastSeenAt:now,staleAt:now},updatedAt:now};accepted.push(superseded,candidate.kind==="preference"?{...candidate,status:candidate.origin==="inferred"?"candidate":"active",supersedesId:conflict.id}:{...candidate,status:"active",supersedesId:conflict.id});continue;}
...
function conflicts(a:WarmMemory,b:WarmMemory){if(a.kind!==b.kind||a.scope.type!==b.scope.type||a.scope.id!==b.scope.id||a.status!=="active")return false;if(a.kind==="preference"&&b.kind==="preference")return a.dimension===b.dimension&&a.applicability===b.applicability&&a.value!==b.value;if(a.kind!=="preference"&&b.kind!=="preference")return a.kind==="fact"&&b.kind==="fact"&&canonicalSubjectPredicate(a)===canonicalSubjectPredicate(b)&&canonicalFingerprint(a)!==canonicalFingerprint(b);return false;}
