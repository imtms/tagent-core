...
export interface JobQueuePort { enqueue(request: CaptureRequest): Promise<CaptureJob>; claim(owner: string, leaseMs: number): Promise<CaptureJob | null>; complete(id: string, result?: { proposalCount: number; persistedCount: number }): Promise<void>; fail(id: string, errorCode: string, retryable: boolean): Promise<void>; listJobs?(scopes: MemoryScope[], limit?: number): Promise<CaptureJob[]>; getJob?(id: string): Promise<CaptureJob | null> }
