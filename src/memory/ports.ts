...
export interface JobQueuePort { enqueue(request: CaptureRequest): Promise<CaptureJob>; claim(owner: string, leaseMs: number): Promise<CaptureJob | null>; renew(id: string, owner: string, leaseToken: string, fencingToken: number, leaseMs: number): Promise<boolean>; complete(id: string, owner: string, leaseToken: string, fencingToken: number, result?: { proposalCount: number; persistedCount: number }): Promise<boolean>; fail(id: string, owner: string, leaseToken: string, fencingToken: number, errorCode: string, retryable: boolean): Promise<boolean>; listJobs?(scopes: MemoryScope[], limit?: number): Promise<CaptureJob[]>; getJob?(id: string): Promise<CaptureJob | null> }
