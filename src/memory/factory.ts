...
export interface MemoryRuntimeConfig { enabled:boolean; backend:"memory"|"postgres"; postgresUrl?:string; coldBackend:"local"|"s3"; coldPath:string; s3Bucket?:string; s3Prefix?:string; s3Endpoint?:string; s3Region?:string; s3ForcePathStyle?:boolean; workerIntervalMs:number; maintenanceIntervalMs:number; workspaceScopeId:string; coldMinimumRecords:number; warmAfterMs:number; hotTtlMs:number }
...
export async function createMemoryRuntime(config:MemoryRuntimeConfig,store:Store){
  ...
  if(config.backend==="postgres"&&!config.postgresUrl)throw new Error("TAGENT_MEMORY_POSTGRES_URL is required for postgres memory backend");
  const adapter=config.backend==="postgres"?new PostgresMemoryAdapter(config.postgresUrl!):new InMemoryMemoryAdapter(); if(adapter instanceof PostgresMemoryAdapter)await adapter.migrate();
  const blobs=config.coldBackend==="s3"?new S3BlobStore({bucket:config.s3Bucket!,prefix:config.s3Prefix,clientConfig:{endpoint:config.s3Endpoint,region:config.s3Region??"us-east-1",forcePathStyle:config.s3ForcePathStyle}}):new LocalBlobStore(path.resolve(config.coldPath));
  ...
  return{service,adapter,worker,lifecycle,consolidator,reconciler,start(){worker.start();},async close(){worker.stop();if(adapter instanceof PostgresMemoryAdapter)await adapter.close();}};
}
