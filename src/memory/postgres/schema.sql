...
CREATE TABLE IF NOT EXISTS memory.records (
 ...
 importance real NOT NULL, source_refs jsonb NOT NULL, valid_from bigint, valid_to bigint, supersedes_id uuid, expires_at bigint,
 created_at bigint NOT NULL, updated_at bigint NOT NULL, search_document tsvector GENERATED ALWAYS AS (to_tsvector('simple', coalesce(title,'')||' '||coalesce(summary,'')||' '||coalesce(content,''))) STORED
);
...
CREATE TABLE IF NOT EXISTS memory.preferences (
 ...
 source_refs jsonb NOT NULL, supersedes_id uuid, expires_at bigint, created_at bigint NOT NULL, updated_at bigint NOT NULL,
