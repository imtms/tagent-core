...
function looksLikeOperationalRequest(content:string){const focus=content.replace(/^user:\s*/i,"").trim();return /[?？]$/.test(focus)||/^(?:为什么|为何|怎么|如何|是否|能否|可否|请|帮我|麻烦|检查|审计|排查|修复|实现|运行|执行|部署|合并|查看|确认|分析|调查|why|how|can you|could you|please|check|audit|debug|fix|implement|run|deploy|merge)/i.test(focus)||/(?:请|帮我|麻烦).{0,12}(?:检查|审计|排查|修复|实现|运行|执行|部署|合并|查看|确认|分析|调查)/i.test(focus);}
