...
export function createGoalApi(request: GoalApiRequest) {
  return {
    ...
    addWorkspaceGoalRoadmap: (goalId: string, content: WorkspaceGoalRoadmap) => request(`/api/v1/console/workspace-goals/${goalId}/roadmaps`, { method: "POST", body: JSON.stringify({ content, actorId: "web_console" }) }, (payload) => decodeAbi(ConsoleWorkspaceGoalSchema, payload)),
    generateWorkspaceGoalRoadmap: (goalId: string) => request(`/api/v1/console/workspace-goals/${goalId}/roadmap/generate`, { method: "POST", body: JSON.stringify({ actorId: "web_console" }) }, (payload) => decodeAbi(ConsoleWorkspaceGoalSchema, payload)),
