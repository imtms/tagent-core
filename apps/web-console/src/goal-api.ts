...
export function createGoalApi(request: GoalApiRequest) {
  return {
    ...
    addWorkspaceGoalPlan: (goalId: string, content: WorkspaceGoalPlan) => request(`/api/v1/console/workspace-goals/${goalId}/plans`, { method: "POST", body: JSON.stringify({ content, requestId: createRequestId(), actorId: "web_console" }) }, (payload) => payload as Record<string, unknown>),
