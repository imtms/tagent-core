...
export function createSkillApi(request: Request) {
  ...
  const workspaceSkills = async (workspaceId: string): Promise<SkillRevision[]> => request(`/api/v1/operator/workspaces/${encodeURIComponent(workspaceId)}/skills?limit=32`, undefined, (payload) => withCoreAbi((abi) => {
    const data = abi.decodeAbi(abi.OperatorWorkspaceSkillsResponseSchema.properties.data, payload);
