...
CREATE INDEX memory_records_scope ON memory.records(scope_type,scope_id,status);
CREATE INDEX memory_records_topics ON memory.records USING gin(topic_ids);
CREATE INDEX memory_records_fts ON memory.records USING gin(search_document);
CREATE INDEX memory_records_trgm ON memory.records USING gin ((coalesce(title,'')||' '||coalesce(summary,'')||' '||coalesce(content,'')) gin_trgm_ops);
...
CREATE INDEX memory_preferences_scope ON memory.preferences(scope_type,scope_id,status);
CREATE INDEX memory_preferences_topics ON memory.preferences USING gin(topic_ids);
CREATE INDEX memory_preferences_fts ON memory.preferences USING gin(search_document);
CREATE INDEX memory_preferences_trgm ON memory.preferences USING gin ((coalesce(dimension,'')||' '||coalesce(value,'')||' '||coalesce(summary,'')) gin_trgm_ops);
CREATE TABLE memory.entities (id text PRIMARY KEY,type text NOT NULL,canonical_name text NOT NULL,aliases text[] NOT NULL,scope_type text NOT NULL,scope_id text NOT NULL,updated_at bigint NOT NULL);
CREATE INDEX memory_entities_scope ON memory.entities(scope_type,scope_id);
...
CREATE TABLE memory.embeddings (ref_type text NOT NULL CHECK(ref_type IN ('hot_record','warm_record','topic_descriptor')),ref_id text NOT NULL,scope_type text NOT NULL,scope_id text NOT NULL,kind text NOT NULL,generation text NOT NULL,embedding vector NOT NULL,content_hash text,updated_at bigint NOT NULL,PRIMARY KEY(ref_type,ref_id,generation));
CREATE INDEX memory_embeddings_scope ON memory.embeddings(scope_type,scope_id,kind,generation);
CREATE TABLE memory.capture_jobs (id uuid PRIMARY KEY,idempotency_key text NOT NULL UNIQUE,request jsonb NOT NULL,status text NOT NULL,attempts integer NOT NULL DEFAULT 0,lease_owner text,lease_until bigint,lease_token uuid,fencing_token bigint NOT NULL DEFAULT 0,error_code text,extracted_count integer,proposal_count integer,persisted_count integer,filter_reasons jsonb NOT NULL DEFAULT '{}',created_at bigint NOT NULL,updated_at bigint NOT NULL);
CREATE INDEX memory_jobs_claim ON memory.capture_jobs(status,lease_until,created_at);
CREATE TABLE memory.policy_receipts (id bigserial PRIMARY KEY,action text NOT NULL,subject_id text NOT NULL,scope_type text,scope_id text,decision text NOT NULL,reason_codes text[] NOT NULL,payload_hash text,policy_version text NOT NULL,created_at bigint NOT NULL);
CREATE TABLE memory.outbox (id bigserial PRIMARY KEY,event_type text NOT NULL,payload jsonb NOT NULL,status text NOT NULL DEFAULT 'pending',created_at bigint NOT NULL,processed_at bigint);

CREATE TABLE memory.reindex_jobs (
...
CREATE INDEX memory_reindex_claim ON memory.reindex_jobs(status,lease_until,created_at);
CREATE TABLE memory.embedding_generations (
...
CREATE UNIQUE INDEX memory_one_active_generation ON memory.embedding_generations(scope_type,scope_id) WHERE status='active';
CREATE TABLE memory.worker_heartbeats (
...
CREATE INDEX memory_metrics_lookup ON memory.metrics(scope_type,scope_id,name,created_at DESC);
CREATE TABLE memory.degraded_events (
 id bigserial PRIMARY KEY, scope_type text NOT NULL, scope_id text NOT NULL, reason text NOT NULL, created_at bigint NOT NULL
);
CREATE TABLE memory.recall_feedback (
...
CREATE INDEX memory_feedback_record ON memory.recall_feedback(record_id,created_at DESC);
CREATE TABLE memory.governance_receipts (
...
CREATE TABLE memory.core_snapshots (
