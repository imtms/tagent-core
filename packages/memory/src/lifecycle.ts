...
export class MemoryLifecycle {
  ...
  async integrate(access:AccessContext,proposal:ExtractionProposal){
    ...
    for(const raw of proposal.records){
      ...
      if(duplicate){const merged=mergeDuplicate(duplicate,candidate),activeConflicts=merged.status==="active"?working.filter((item)=>item.id!==merged.id&&conflicts(item,merged)):[],superseded=activeConflicts.map((item)=>supersede(item,now));accepted.push(...superseded,merged);replaceWorking(working,duplicate,merged);for(let index=0;index<activeConflicts.length;index++)replaceWorking(working,activeConflicts[index],superseded[index]);continue;}
      const conflict=working.find((item)=>conflicts(item,candidate));
      if(conflict){const superseded=supersede(conflict,now),replacement:WarmMemory=candidate.kind==="preference"?{...candidate,status:candidate.origin==="inferred"?"candidate":"active",supersedesId:conflict.id}:{...candidate,status:"active",supersedesId:conflict.id};accepted.push(superseded,replacement);replaceWorking(working,conflict,superseded);working.push(replacement);continue;}
...
function supersede(record:WarmMemory,now:number):WarmMemory{const lifecycle=state(record);return record.kind==="preference"?{...record,status:"superseded",lifecycle:{...lifecycle,lastSeenAt:now,staleAt:now},updatedAt:now}:{...record,status:"superseded",validTo:now,lifecycle:{...lifecycle,lastSeenAt:now,staleAt:now},updatedAt:now};}
