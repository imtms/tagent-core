...
import { MemoryHistoryBackfill } from "./history-backfill.js";
...
export async function createMemoryRuntime(config:MemoryRuntimeConfig,source:MemoryRuntimeSourcePort,semanticJudge?:SemanticMemoryJudgePort){
  ...
  const llmExtractor=config.extractorProvider === "hybrid"?new LlmExtractor({baseUrl:config.extractorBaseUrl!,resolveApiKey:config.resolveExtractorApiKey!,model:config.extractorModel!,maxRetries:1}):undefined;
  const extractor=new HybridExtractor(ruleExtractor,llmExtractor);
  const extractorProbe:import("./ports.js").ProbePort|undefined=llmExtractor?{probe:async()=>{const started=Date.now();try{await llmExtractor.extract("user: readiness probe, do not persist",[],access.scopes[0]);return{ok:true,latencyMs:Date.now()-started};}catch(error){return{ok:false,latencyMs:Date.now()-started,error:String(error)}}}}:undefined;
  const core=new CoreMemorySnapshotService(adapter,adapter);
  const service=new MemoryService({records:adapter,vectors:adapter,graph:adapter,topics:adapter,blobs,embeddings,jobs:adapter,policy,reindex:adapter,operations:adapter,embeddingProbe:probe,extractorProbe,coreSnapshots:{get:a=>core.get(a),generate:(a,o)=>core.generate(a,o),update:(a,m)=>core.update(a,m)}},config.recallThresholds);
