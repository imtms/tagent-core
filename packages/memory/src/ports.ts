...
export interface ReindexFinalizationRequest {
  jobId:string;
  owner:string;
  leaseToken:string;
  fencingToken:number;
  scope:MemoryScope;
  generation:string;
  activeRefs:Array<Pick<VectorDocument,"refType"|"refId">>;
  checkpoint:ReindexJob["checkpoint"];
  expected:number;
  indexed:number;
  skipped:number;
}
export interface ReindexJobPort { enqueueReindex(scope:MemoryScope,generation:string):Promise<ReindexJob>; claimReindex(owner:string,leaseMs:number):Promise<ReindexJob|null>; renewReindex(id:string,owner:string,leaseToken:string,fencingToken:number,leaseMs:number):Promise<boolean>; upsertReindexVectors(id:string,owner:string,leaseToken:string,fencingToken:number,documents:VectorDocument[]):Promise<boolean>; checkpointReindex(job:ReindexJob,owner:string,leaseToken:string,fencingToken:number):Promise<boolean>; finalizeReindex(request:ReindexFinalizationRequest):Promise<boolean>; failReindex(id:string,owner:string,leaseToken:string,fencingToken:number,errorCode:string):Promise<boolean>; listReindexJobs(scopes:MemoryScope[],limit?:number):Promise<ReindexJob[]>; getGeneration(scope:MemoryScope,generation?:string):Promise<EmbeddingGenerationState|null> }
