...
export interface MemoryDurableUserMessageView { id: number; content: string }
...
export interface TopicCatalogPort {
  ...
  getDescriptors(topicIds: string[], scopes: MemoryScope[]): Promise<TopicDescriptor[]>; listDescriptors(scopes: MemoryScope[], kinds?: MemoryKind[], limit?: number): Promise<TopicDescriptor[]>; countTopicSummary?(scopes: MemoryScope[]): Promise<{ topics: number; coldTopics: number }>; stageRevision(revision: ColdRevision): Promise<void>; publishRevision(topicId: string, revisionId: string): Promise<void>; abandonRevision(revisionId: string): Promise<void>; listStagedRevisions(olderThan: number, limit: number): Promise<ColdRevision[]>; getCurrentRevision(topicId: string, scopes: MemoryScope[]): Promise<ColdRevision | null>; forgetTopics(topicIds: string[], scopes: MemoryScope[], options?: { reason?: string; purgeAfter?: number }): Promise<ColdRevision[]>; restoreTopics?(topicIds: string[], scopes: MemoryScope[]): Promise<number>; purgeDeletedTopics?(scopes: MemoryScope[], now: number, limit: number): Promise<ColdRevision[]>;
}
...
export interface CoreSnapshotPort { getSnapshot(scope:MemoryScope):Promise<CoreMemorySnapshot|null>; putSnapshot(snapshot:CoreMemorySnapshot):Promise<void> }
