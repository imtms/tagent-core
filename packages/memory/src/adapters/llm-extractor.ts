...
import { canonicalOrganizationTopic, hasMalformedNegation, isControlPlaneText, isDurableMemory, isOrganizationDerivedOrMetadata, isOrganizationEvidence, isOneOffRequest } from "../quality.js";
...
export interface LlmExtractorOptions {baseUrl:string;resolveApiKey:()=>Promise<string|undefined>;model:string;timeoutMs?:number;maxRetries?:number}
...
export class LlmExtractor implements ExtractorPort {
  ...
  async extract(content:string,sourceRefs:SourceReference[],scope:MemoryScope):Promise<ExtractionProposal>{
    ...
    const organizationInput=/组织架构|公司架构|汇报关系/.test(content);
    for(const raw of result.items??[]){
      const item=validateItem(raw);if(!item)continue;
      const positiveText=`${item.subject} ${item.predicate} ${item.object}`;
      const text=item.polarity==="negative"?renderNegative(item.subject,item.predicate,item.object):positiveText;
      if(isControlPlaneText(item.summary)||isControlPlaneText(text)||hasMalformedNegation(`${item.summary} ${text}`))continue;
      if(organizationInput&&isOrganizationDerivedOrMetadata(`${item.summary} ${text}`)&&!isOrganizationEvidence(text))continue;
      const subject=node(item.subject,item.subject==="用户"?"user":"person",scope);const object=node(item.object,"concept",scope);nodes.set(subject.id,subject);nodes.set(object.id,object);
      const topicId=organizationInput&&isOrganizationEvidence(text)?canonicalOrganizationTopic(scope):`${scope.type}.${scope.id}.${item.kind}.${slug(item.subject)}.${slug(item.dimension??item.predicate)}`.slice(0,180);
      const semantic={subject:item.subject,predicate:item.predicate,object:item.object,polarity:item.polarity??"unknown" as const};
      const common={id:randomUUID(),tier:"hot" as const,scope,topicIds:[topicId],entityIds:[subject.id,object.id],semantic,status:item.status??(item.origin==="inferred"?"candidate":"active"),confidence:item.confidence,sourceRefs,createdAt:now,updatedAt:now};
      const record:WarmMemory=item.kind==="preference"?{...common,kind:"preference",dimension:item.dimension??"general",value:text,summary:item.summary,applicability:item.applicability??"workspace",strength:item.importance??.85,origin:item.origin??"explicit"}:{...common,kind:item.kind,title:`${label(item.kind)}: ${item.summary}`.slice(0,100),content:text,summary:item.summary,importance:item.importance??.75,validFrom:item.validFrom,validTo:item.validTo};
      if(!isDurableMemory(record))continue;records.push(record);
      const existing=topics.get(topicId);topics.set(topicId,{topicId,kind:item.kind,scope,title:organizationInput&&isOrganizationEvidence(text)?"某公司组织架构":`${item.subject}: ${item.dimension??item.predicate}`,description:[existing?.description,item.summary].filter(Boolean).join(" ").slice(0,800),aliases:[...new Set([...(existing?.aliases??[]),item.subject,item.object,item.predicate,item.dimension??"",...(organizationInput?["组织架构","公司架构","汇报关系","共同上级"]:[])].filter(Boolean))],entityIds:[...new Set([...(existing?.entityIds??[]),subject.id,object.id])],relatedTopicIds:[],embeddingText:[existing?.embeddingText,`${item.subject} ${item.predicate} ${item.object} ${item.summary}`].filter(Boolean).join("\n").slice(0,1600),status:"active",createdAt:existing?.createdAt??now,updatedAt:now});
  ...
  private async call(content:string):Promise<LlmResult>{
    ...
    for(let attempt=0;attempt<=retries;attempt++){const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),this.options.timeoutMs??60_000);try{const apiKey=await this.options.resolveApiKey();if(!apiKey)throw new Error("Extractor API credential is unavailable");const response=await fetch(`${this.baseUrl}/chat/completions`,{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${apiKey}`},body:JSON.stringify({model:this.options.model,messages:[{role:"user",content:prompt}],temperature:0,response_format:{type:"json_object"}}),signal:controller.signal});const text=await response.text();if(!response.ok)throw new Error(`Extractor API ${response.status}: ${text.slice(0,500)}`);const parsed=JSON.parse(text) as {choices?:Array<{message?:{content?:string}}>};return JSON.parse(parsed.choices?.[0]?.message?.content??"{}") as LlmResult;}catch(error){last=error;if(attempt===retries)throw error;await new Promise((resolve)=>setTimeout(resolve,500*2**attempt));}finally{clearTimeout(timer);}}throw last;}
}

export class HybridExtractor implements ExtractorPort {constructor(private readonly rule:ExtractorPort,private readonly llm?:ExtractorPort){}async extract(content:string,refs:SourceReference[],scope:MemoryScope){const focus=focusContent(content);const fast=await this.rule.extract(focus,refs,scope);const contextualFood=await contextualFoodFallback(this.rule,content,refs,scope);const contextualRelations=contextualRelationshipFallback(content,refs,scope);const deterministic=merge(merge(fast,contextualFood),contextualRelations);if(!this.llm||looksLikeOperationalRequest(focus))return deterministic;try{const semantic=await this.llm.extract(content,refs,scope);return merge(deterministic,semantic);}catch(error){if(deterministic.records.length||deterministic.topics.length||deterministic.nodes.length||deterministic.edges.length)return deterministic;throw error;}}}
...
function validateItem(item:LlmItem){if(!item||!["fact","preference","episode","procedure"].includes(item.kind)||!clean(item.subject)||!clean(item.predicate)||!clean(item.object)||!clean(item.summary)||!Number.isFinite(item.confidence))return undefined;const rawSubject=clean(item.subject);if(/^(?:我|本人|用户|user)不$/i.test(rawSubject))return undefined;const userSubject=/^(?:我|本人|用户|user)$/i.test(rawSubject);return{...item,subject:userSubject?"用户":rawSubject,predicate:clean(item.predicate),object:clean(item.object),summary:clean(item.summary),polarity:item.polarity,confidence:clamp(item.confidence),importance:clamp(item.importance??.75)};}
function renderNegative(subject:string,predicate:string,object:string){if(/^(?:有|存在|发生|具备|包含)$/.test(predicate))return `${subject} 不${predicate} ${object}`;if(/^(?:喜欢|偏好|接受|使用|支持|同意)$/.test(predicate))return `${subject} 不${predicate} ${object}`;return `${subject} 未${predicate} ${object}`;}
...
function looksLikeOperationalRequest(content:string){const focus=content.replace(/^user:\s*/i,"").trim();return isOneOffRequest(focus);}
...
function contextualRelationshipFallback(content:string,refs:SourceReference[],scope:MemoryScope):ExtractionProposal{
 ...
 const addFact=(subjectName:string,predicate:string,objectName:string,summary:string,aliases:string[],objectType="place")=>{
  ...
  topics.set(topicId,{topicId,kind:"fact",scope,title:predicate==="lives_in"?`${subjectName}的住所`:`${subjectName}的邻居关系`,description:summary,aliases:[subjectName,objectName,...aliases],entityIds:[subject.id,object.id],relatedTopicIds:[],embeddingText:`${subjectName} ${predicate} ${objectName} ${summary}`,status:"active",createdAt:now,updatedAt:now});
