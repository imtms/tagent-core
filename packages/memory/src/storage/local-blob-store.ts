...
export class LocalBlobStore implements BlobStorePort {
  ...
  async putImmutable(key: string, body: string, _metadata: Record<string,string>) { const file=this.resolve(key); await mkdir(path.dirname(file),{recursive:true}); const checksum=createHash("sha256").update(body).digest("hex"); try { const existing=await readFile(file,"utf8"); if(createHash("sha256").update(existing).digest("hex")===checksum)return{checksum,byteLength:Buffer.byteLength(existing)}; throw new Error(`Immutable object already exists with different content: ${key}`); } catch (error) { if((error as NodeJS.ErrnoException).code!=="ENOENT")throw error; } await writeFile(file,body,{encoding:"utf8",flag:"wx",mode:0o600}); return { checksum, byteLength:Buffer.byteLength(body) }; }
