...
export function buildRuntimeAttemptContext(
  ...
  const workspaceGoalInstruction = run.contract?.workspaceGoal?.mode === "roadmap"
    ? "The immutable Workspace Goal snapshot is authoritative direction. Execute only the targeted approved Roadmap item. Its mapped Goal criteria identify cumulative Goal-level evidence this Run may advance; they are not additional TaskRun completion requirements. Complete only the Roadmap item's own outcome and verification. If the request conflicts with the Goal, scope, non-goals, or approved slice, do not mutate the Workspace; explain the conflict or request a deliberate Goal/Roadmap revision."
