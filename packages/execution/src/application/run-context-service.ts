  ...
  public publishContextEvents(runId: RunId, assembly: ContextAssembly & { memoryContextItems?: ContextManifestItem[]; projectContextItems?: ContextManifestItem[]; projectContextHash?: string }) {
    ...
    if (run) {
      const items: ContextManifestItem[] = [
        ...
        ...(run.contract ? [{ kind: "taskrun_contract" as const, sourceId: run.requestId, selected: true, reason: "active TaskRun execution contract", estimatedTokens: estimateContextTokens(JSON.stringify({ ...run.contract, workspaceGoal: undefined, skill: undefined })) }] : []),
        ...
        ...(run.contract?.skill ? [{ kind: "skill" as const, sourceId: run.contract.skill.revisionId, selected: true, reason: "Core-selected immutable Skill revision", estimatedTokens: estimateContextTokens(run.contract.skill.content), metadata: { name: run.contract.skill.name, revision: run.contract.skill.revision, sha256: run.contract.skill.sha256, filePath: run.contract.skill.filePath } }] : []),
