...
export function workspaceGoalNextAction(input: {
  ...
  if (input.currentRunId && ["blocked", "interrupted"].includes(input.currentRunStatus ?? "")) return action("user", "resolve_problem", "A Goal TaskRun needs attention", "Open the blocked TaskRun, resolve its blocker, and resume it.", "Open task", null, input.currentRunId);
  ...
  if (nextItemId && nextProgress?.status === "blocked" && !nextProgress.retryable && nextProgress.runId) {
...
export function planWorkspaceGoalDecision(input:{goal:WorkspaceGoal;revision:WorkspaceGoalRevision;decision:WorkspaceGoalDecisionInput;hasOtherGuidingGoal:boolean}){
  ...
  if(decision.kind==="approve_goal"&&goal.definition?.id!==revision.id)throw new Error("workspace Goal definition revision is stale");
  if(decision.kind==="approve_roadmap"&&goal.roadmap?.id!==revision.id)throw new Error("workspace Goal Roadmap revision is stale");
  if(["pause","resume","close"].includes(decision.kind)&&goal.activeDefinitionRevisionId!==revision.id)throw new Error("workspace Goal lifecycle decision revision is stale");
  if(decision.kind==="cancel"&&(goal.activeDefinitionRevisionId??goal.definition?.id)!==revision.id)throw new Error("workspace Goal lifecycle decision revision is stale");
