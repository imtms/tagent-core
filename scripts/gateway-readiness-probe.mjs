...
const REQUIRED_CAPABILITY_PROFILES = {
  "operator.session-settings.v1": [
    "operator.session_settings.get", "operator.session_settings.update",
  ],
  "operator.session-inbox.v1": [
    "operator.session_inbox.list", "operator.session_inbox.reorder", "operator.session_inbox.update",
    "operator.session_inbox.decide", "operator.session_inbox.merge", "operator.session_inbox.delete",
    "operator.session_inbox.start", "operator.session_inbox.retry_launch", "operator.operations.get",
  ],
  "operator.context-manifest.v1": ["operator.context_manifests.list"],
  "operator.skills.v1": [
    "operator.skills.list", "operator.skills.get", "operator.skills.revisions.list",
    "operator.skills.create", "operator.skills.update", "operator.skills.delete",
    "operator.workspace_skills.list", "operator.workspace_skills.replace",
  ],
  "admin.memory.v1": [
...
function readReceiptHealth(db, now) {
  ...
  return {
    ...
    capabilityProfiles: read("profile_operation_receipts"),
  };
}
...
function recoveryCompatible(endpoint) {
  const recovery = endpoint?.recovery;
  const expectedKind = EXACT_REPLAY_PROFILE_ENDPOINTS.has(endpoint?.id)
    ? "exact_replay_readback"
    : DURABLE_RECEIPT_PROFILE_ENDPOINTS.has(endpoint?.id)
      ? "durable_receipt_lookup"
      : "none";
  if (recovery?.kind !== expectedKind || recovery?.automaticUnknownReplay !== false) return false;
  if (expectedKind === "exact_replay_readback") {
    return recovery.idempotencyKeyRequired === true
      && recovery.revisionPreconditionRequired === true
      && recovery.operationLookupPath === null
      && recovery.interruptedEffectState === "not_applicable";
  }
  if (expectedKind === "durable_receipt_lookup") {
    return recovery.idempotencyKeyRequired === true
      && recovery.revisionPreconditionRequired === false
      && /^\/api\/v1\/(?:operator|admin)\/operations\/:requestId$/.test(recovery.operationLookupPath)
      && recovery.interruptedEffectState === "outcome_unknown";
  }
  return recovery.idempotencyKeyRequired === false
    && recovery.revisionPreconditionRequired === false
    && recovery.operationLookupPath === null
    && recovery.interruptedEffectState === "not_applicable";
}

function profileDetailCompatible(detail, expectedId, expectedEndpointIds) {
  const profile = detail?.data?.profile;
  const authorization = detail?.data?.authorization;
  const paginated = expectedId !== "operator.session-settings.v1";
  return profile?.id === expectedId
    && profile?.version === "1.0"
    && sameStrings(profile?.endpointIds, expectedEndpointIds)
    && Array.isArray(profile?.endpoints)
    && profile.endpoints.length === expectedEndpointIds.length
    && profile.endpoints.every((endpoint, index) => endpoint?.id === expectedEndpointIds[index]
      && Array.isArray(endpoint?.requiredScopes) && endpoint.requiredScopes.length > 0
      && recoveryCompatible(endpoint))
    && profile?.pagination?.style === (paginated ? "opaque_cursor" : "none")
    && profile?.pagination?.cursorVersion === (paginated ? "1" : null)
    && profile?.pagination?.cursorOpaque === paginated
    && profile?.pagination?.cursorExpiry === false
    && profile?.pagination?.cursorSurvivesRestart === true
    && profile?.pagination?.membershipConsistency === (paginated ? "snapshot" : "not_applicable")
    && profile?.pagination?.defaultLimit === (paginated ? 50 : null)
    && profile?.pagination?.maximumLimit === (paginated ? 200 : null)
    && profile?.retention?.automaticDeletion === false
    && profile?.retention?.tombstones === false
    && profile?.retention?.missingResourceStatus === 404
    && profile?.compatibility?.additiveChangesRequireMinor === true
    && profile?.compatibility?.incompatibleChangesRequireMajor === true
    && profile?.compatibility?.unknownResponseFields === "rejected"
    && authorization?.status === "available"
    && sameStrings(authorization?.availableEndpointIds, expectedEndpointIds)
    && sameStrings(authorization?.missingScopes, []);
}

async function readCapabilityProfiles(url, token) {
  try {
    const headers = token ? { Authorization: `Bearer ${token}` } : {};
    const response = await fetch(url, { headers, signal: AbortSignal.timeout(5_000) });
    const body = await response.json();
    const profiles = body?.data?.profiles;
    const expectedEntries = Object.entries(REQUIRED_CAPABILITY_PROFILES);
    let compatible = response.ok
      && Array.isArray(profiles)
      && profiles.length === expectedEntries.length
      && profiles.every((summary, index) => {
        const [profileId, endpointIds] = expectedEntries[index];
        return summary?.id === profileId
          && summary?.version === "1.0"
          && summary?.authorization?.status === "available"
          && sameStrings(summary?.authorization?.availableEndpointIds, endpointIds)
          && sameStrings(summary?.authorization?.missingScopes, []);
      });
    const details = {};
    if (compatible) {
      for (const [profileId, endpointIds] of expectedEntries) {
        const detailUrl = new URL(`/api/v1/capability-profiles/${encodeURIComponent(profileId)}`, url);
        const detailResponse = await fetch(detailUrl, { headers, signal: AbortSignal.timeout(5_000) });
        const detail = await detailResponse.json();
        details[profileId] = detail?.data ?? null;
        if (!detailResponse.ok || !profileDetailCompatible(detail, profileId, endpointIds)) compatible = false;
      }
    }
    return { reachable: true, status: response.status, compatible, data: body?.data, details };
  } catch (error) {
    return { reachable: false, status: null, compatible: false, error: error instanceof Error ? error.message : String(error) };
  }
}
...
function severityFor(snapshot) {
  ...
  if (snapshot.schemaVersion !== EXPECTED_SCHEMA_VERSION
    ...
    || !snapshot.capabilityProfiles.compatible
    ...
    || snapshot.receipts?.capabilityProfiles === null
...
async function main() {
  ...
  const capabilityProfilesUrl = process.env.TAGENT_CAPABILITY_PROFILES_URL?.trim()
    || capabilitiesUrl.replace(/\/capabilities(?:\?.*)?$/, "/capability-profiles");
  ...
  const [health, capabilities, operatorReadCapabilities, capabilityProfiles] = await Promise.all([
    readHealth(healthUrl),
    readCapabilities(capabilitiesUrl, coreToken),
  ...
  if (!capabilityProfiles.reachable) reasons.push("capability_profiles_unreachable");
  else if (!capabilityProfiles.compatible) reasons.push("capability_profiles_incompatible");
  ...
  if (databaseSnapshot.receipts.capabilityProfiles === null) reasons.push("profile_receipts_unavailable");
  else {
    if (databaseSnapshot.receipts.capabilityProfiles.started > 0
      && (databaseSnapshot.receipts.capabilityProfiles.oldestUncertainAgeMs ?? 0) >= RECEIPT_UNCERTAIN_CRITICAL_AGE_MS) {
      reasons.push("profile_receipts_stale_started");
    }
