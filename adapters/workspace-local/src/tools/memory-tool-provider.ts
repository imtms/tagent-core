...
export class MemoryToolProvider implements ToolProvider {
  ...
  provideTools(): readonly RuntimeTool[] {
    ...
    if (!memory) return [];
    const search: RuntimeTool<Static<typeof SearchSchema>, Record<string, unknown>> = { name: "memory_search", label: "Search memory", description: "Search long-term memory when automatic recall is insufficient. Returns cards, topic IDs, confidence and provenance routes.", parameters: SearchSchema, execute: async (_id, params) => textResult(JSON.stringify(await memory.search(params.query, params.kinds, params.maxResults), null, 2)) };
    const topic: RuntimeTool<Static<typeof TopicSchema>, Record<string, unknown>> = { name: "memory_topic_get", label: "Read memory topic", description: "Read one complete canonical Cold Topic page by exact topic ID.", parameters: TopicSchema, execute: async (_id, params) => { const value = await memory.getTopic(params.topicId); if (!value) throw new Error("Memory topic not found"); return textResult(value.body, { topicId: params.topicId, revision: value.revision, checksum: value.checksum }); } };
    const record: RuntimeTool<Static<typeof RecordSchema>, Record<string, unknown>> = { name: "memory_record_get", label: "Read memory record", description: "Read one full memory record including source references, provenance, status, validity and canonical semantics.", parameters: RecordSchema, execute: async (_id, params) => { const value = await memory.getRecord(params.id); if (!value) throw new Error("Memory record not found"); return textResult(JSON.stringify(value, null, 2)); } };
    const forget: RuntimeTool<Static<typeof ForgetSchema>, Record<string, unknown>> = { name: "memory_forget", label: "Forget memory", description: "Forget specified memory record IDs or Topic IDs. Use only when the user explicitly requests deletion or correction.", parameters: ForgetSchema, executionMode: "sequential", policy: { operationType: "tool.memory_forget", workspaceAccess: "mutation", externalAction: true }, execute: async (_id, params) => { if (!params.ids?.length && !params.topicIds?.length) throw new Error("Memory forget requires at least one record ID or Topic ID"); return textResult(JSON.stringify(await memory.forget(params), null, 2)); } };
