...
describe("Execution workspace package", () => {
  it("publishes only the explicit compiled Execution ABI", () => {
    ...
    expect(execution).toMatchObject({ name: "@tagent/execution", version: "0.8.2", private: true });
    ...
    expect(execution.dependencies).toEqual({ "@tagent/governance": "0.8.2" });
  ...
  it("keeps the explicit implementations in the Execution package", () => {
    ...
    expect(implementationFiles).toHaveLength(58);
