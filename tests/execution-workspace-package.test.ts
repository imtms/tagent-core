...
describe("Execution workspace package", () => {
  it("publishes only the explicit compiled Execution ABI", () => {
    ...
    expect(execution).toMatchObject({ name: "@tagent/execution", version: "0.4.0", private: true });
    ...
    expect(execution.dependencies).toEqual({ "@tagent/governance": "0.4.0" });
  ...
  it("keeps the explicit implementations in the Execution package", () => {
    ...
    expect(implementationFiles).toHaveLength(49);
