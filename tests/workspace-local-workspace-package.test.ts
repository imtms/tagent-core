...
describe("Local workspace adapter package", () => {
  it("publishes a private minimal ABI with exact approved dependencies", () => {
    ...
    expect(manifest).toMatchObject({ name: "@tagent/workspace-local", version: "0.8.23", private: true });
  ...
  it("keeps production consumers on package ABIs and Pi concrete ownership in runtime-pi", () => {
    ...
    expect(piAgentCoreImporters).toEqual(["adapters/runtime-pi/src/pi-runtime.ts"]);
  });
