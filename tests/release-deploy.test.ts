...
const checksumScript = path.resolve("scripts/write-release-checksum.sh");
...
async function releaseDirectory(root: string, commit: string, corrupt: "native" | "manifest" | "syntax" | null = null) {
  ...
  await writeFile(path.join(directory, "node_modules", "@tagent", "admission", "package.json"), JSON.stringify({ name: "@tagent/admission", type: "module", dependencies: { "@tagent/execution": "0.8.1", "@tagent/governance": "0.8.1" } }));
  ...
  await writeFile(path.join(directory, "node_modules", "@tagent", "core-service", "package.json"), JSON.stringify({ name: "@tagent/core-service", type: "module", dependencies: { "@tagent/admission": "0.8.1", "@tagent/execution": "0.8.1", "@tagent/governance": "0.8.1", "@tagent/http-fastify": "0.8.1", "@tagent/learning": "0.8.1", "@tagent/memory": "0.8.1", "@tagent/persistence-sqlite": "0.8.1", "@tagent/runtime-pi": "0.8.1", "@tagent/workspace-local": "0.8.1" } }));
  ...
  await writeFile(path.join(directory, "node_modules", "@tagent", "http-fastify", "package.json"), JSON.stringify({ name: "@tagent/http-fastify", type: "module", dependencies: { "@tagent/abi": "0.8.1", "@tagent/admission": "0.8.1", "@tagent/execution": "0.8.1", "@tagent/governance": "0.8.1" } }));
  await writeFile(path.join(directory, "node_modules", "@tagent", "http-fastify", "dist", "index.js"), "export {};\n");
  await writeFile(path.join(directory, "node_modules", "@tagent", "execution", "package.json"), JSON.stringify({ name: "@tagent/execution", type: "module", dependencies: { "@tagent/governance": "0.8.1" } }));
  await writeFile(path.join(directory, "node_modules", "@tagent", "execution", "dist", "index.js"), "export {};\n");
  await writeFile(path.join(directory, "node_modules", "@tagent", "learning", "package.json"), JSON.stringify({ name: "@tagent/learning", type: "module", dependencies: { "@tagent/admission": "0.8.1", "@tagent/execution": "0.8.1", "@tagent/governance": "0.8.1", "@tagent/memory": "0.8.1" } }));
  ...
  await writeFile(path.join(directory, "node_modules", "@tagent", "persistence-sqlite", "package.json"), JSON.stringify({ name: "@tagent/persistence-sqlite", type: "module", dependencies: { "@tagent/admission": "0.8.1", "@tagent/execution": "0.8.1", "@tagent/governance": "0.8.1", "@tagent/learning": "0.8.1", "@tagent/memory": "0.8.1" } }));
  await writeFile(path.join(directory, "node_modules", "@tagent", "persistence-sqlite", "dist", "index.js"), "export {};\n");
  await writeFile(path.join(directory, "node_modules", "@tagent", "runtime-pi", "package.json"), JSON.stringify({ name: "@tagent/runtime-pi", type: "module", dependencies: { "@tagent/execution": "0.8.1" } }));
  await writeFile(path.join(directory, "node_modules", "@tagent", "runtime-pi", "dist", "index.js"), "export {};\n");
  await writeFile(path.join(directory, "node_modules", "@tagent", "workspace-local", "package.json"), JSON.stringify({ name: "@tagent/workspace-local", type: "module", dependencies: { "@tagent/execution": "0.8.1" } }));
  await writeFile(
...
describe("production release deployment", () => {
  ...
  it("writes portable release checksums that verify outside the build runner path", async () => {
    const buildSource = await readFile(buildScript, "utf8");
    expect(buildSource.split("scripts/write-release-checksum.sh")).toHaveLength(3);
    expect(buildSource).not.toContain('sha256sum "$core_output"');
    expect(buildSource).not.toContain('sha256sum "$web_output"');

    const root = await mkdtemp(path.join(os.tmpdir(), "tagent-checksum-test-"));
    const output = path.join(root, "release output");
    const archive = path.join(output, "tagent-core-test.tar.gz");
    try {
      await mkdir(output);
      await writeFile(archive, "verified archive bytes\n");
      const write = spawnSync("bash", [checksumScript, archive], { encoding: "utf8" });
      expect(write.status, write.stderr).toBe(0);
      expect(await readFile(`${archive}.sha256`, "utf8")).toMatch(/^[a-f0-9]{64} {2}tagent-core-test\.tar\.gz\n$/);
      const verify = spawnSync("sha256sum", ["-c", "tagent-core-test.tar.gz.sha256"], { cwd: output, encoding: "utf8" });
      expect(verify.status, verify.stderr).toBe(0);
      expect(verify.stdout).toContain("tagent-core-test.tar.gz: OK");
    } finally {
