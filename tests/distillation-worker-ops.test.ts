...
import { DistillationWorker, WorkflowService } from "@tagent/learning";
...
describe("automatic distillation worker operations",()=>{
  it("starts on a schedule, processes queued work, and exposes readiness and metrics",async()=>{
    const store=new Store(":memory:");stores.push(store);const service=new WorkflowService(workflowPersistence(store));const session=store.createSession();
  ...
  it("lists dead letters and repairs them for retry",()=>{
    const store=new Store(":memory:");stores.push(store);const service=new WorkflowService(workflowPersistence(store));const session=store.createSession();service.enqueueDistillation(session.id,"broken");
