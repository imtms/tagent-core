...
describe("automatic distillation worker operations",()=>{
  it("starts on a schedule, processes queued work, and exposes readiness and metrics",async()=>{
    const store=new Store(":memory:");stores.push(store);const service=new WorkflowService(store);const session=store.createSession();
    ...
    for(let i=0;i<40&&(store.db.prepare("SELECT status FROM workflow_distillation_jobs").get() as any).status!=="completed";i++)await sleep(5);
    const snapshot=worker.snapshot();expect(snapshot).toMatchObject({running:true,ready:true,owner:"test-worker"});expect(snapshot.metrics.completed).toBe(1);await worker.close();expect(worker.snapshot().ready).toBe(false);
  });

  it("lists dead letters and repairs them for retry",()=>{
    const store=new Store(":memory:");stores.push(store);const service=new WorkflowService(store);const session=store.createSession();service.enqueueDistillation(session.id,"broken");
