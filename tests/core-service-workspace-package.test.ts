...
const expectedDependencies = {
  "@tagent/admission": "0.5.6",
  "@tagent/execution": "0.5.6",
  "@tagent/governance": "0.5.6",
  "@tagent/http-fastify": "0.5.6",
  "@tagent/learning": "0.5.6",
  "@tagent/memory": "0.5.6",
  "@tagent/persistence-sqlite": "0.5.6",
  "@tagent/runtime-pi": "0.5.6",
  "@tagent/workspace-local": "0.5.6",
};
...
describe("Core service application workspace package", () => {
  ...
  it("publishes the private process App through four explicit compiled ABI entries", () => {
    ...
    expect(manifest).toMatchObject({ name: "@tagent/core-service", version: "0.5.6", private: true });
    ...
    expect(root.scripts["dev:server"]).toBe("tsx watch apps/core-service/src/cli.ts");
    expect(root.scripts.start).toBe("node dist/server.js");
    expect(readFileSync(path.join(repoRoot, packageRoot, "dist", "cli.js"), "utf8"))
      .toMatch(/^#!\/usr\/bin\/env node/);
  });

  it("keeps the root server as the only documented CLI compatibility wrapper", () => {
    ...
    const visit = (node: ts.Node) => {
      if (ts.isCallExpression(node) && ts.isIdentifier(node.expression)
        && node.expression.text === "runCoreServiceFromCli") calls.push(node);
      ts.forEachChild(node, visit);
    };
    visit(source);
    expect(moduleSpecifiers(relativePath).sort()).toEqual([
      "@tagent/core-service",
      "@tagent/core-service",
      "node:path",
      "node:url",
    ...
    expect(calls[0].parent.parent.parent.parent.kind).toBe(ts.SyntaxKind.ExpressionStatement);
    expect(text).toContain("if (entry === import.meta.url)");
    expect(text).not.toMatch(/\b(?:Store|CoreLifecycle|AgentService|createApp|acquireCoreInstanceLock|claimCoreWriterConnectionWithRetry)\b/);
    ...
    expect((cli.match(/runCoreServiceFromCli\(\)/g) ?? [])).toHaveLength(1);
  });
  ...
  it("wires one guarded persistence graph through Memory, Learning, Core, and HTTP", () => {
    ...
    for (const sentinel of [
      ...
      "createMemoryRuntime(config.memory, persistence.memory, semanticJudge)",
  ...
  it("keeps dynamic release materialization and the core-service deploy fixture complete", () => {
    ...
    expect(fixture).toContain('"@tagent/core-service": "0.5.6"');
    expect(fixture).toContain('"@tagent", "core-service", "dist"');
    expect(fixture).toContain("node_modules/@tagent/core-service/dist/index.js");
  });

  it("resolves all four public entries through compiled Node ESM without starting the service", () => {
    const script = `
      ...
      if (typeof root.bootstrapCore !== "function" || typeof root.runCoreServiceFromCli !== "function") process.exit(1);
      if (typeof application.AgentService !== "function" || typeof application.CoreApplicationCoordinator !== "function") process.exit(1);
