...
describe("v1 API contracts", () => {
  it("creates Sessions idempotently, exposes GET, and publishes capabilities", async () => {
    ...
    expect(capabilities).toMatchObject({ releaseVersion: "0.5.4", persistenceSchemaVersion: 42, interactions: { approvalResolution: true, userInputSubmission: true }, operator: { roadmapGenerationIdempotent: true } });
  });
