...
function createAdapter(store: Store) {
  return new LegacyStoreAdapter(store, transactionalUnitOfWork(store));
}
...
describe("LegacyStoreAdapter", () => {
  ...
  it("rolls back a Run transition and preserves contiguous event ordering", () => {
    ...
    store.db.exec(`
      ...
      BEFORE INSERT ON learning_projection_outbox
      BEGIN
    ...
    expect(adapter.learningProjections.listPendingLearningProjections()).toEqual([]);
    ...
    expect(adapter.learningProjections.listPendingLearningProjections()).toHaveLength(1);
  });
  ...
  it("rejects stale-owner mutations before Store state changes while allowing queries", () => {
    ...
    const adapter = createGuardedLegacyStoreAdapter(store, staleGuard);
  ...
  it("guards every learning-ledger mutation while retaining stale-owner reads", () => {
    ...
    const adapter = createGuardedLegacyStoreAdapter(store, new WriterFenceGuard(store.db, firstLease.authority));
    const identity = {
      id: "profile-1",
  ...
  it("keeps the guarded mutation bridge synchronous at type and runtime boundaries", () => {
    ...
    const unitOfWork = new GuardedStoreUnitOfWork(new WriterFenceGuard(store.db, lease.authority));
