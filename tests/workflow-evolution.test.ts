...
const stores: Store[] = [];
const make = () => { const store = new Store(":memory:"); stores.push(store); return { store, service: new WorkflowService(store, "test-evaluator-secret") }; };
...
const activate=(service:WorkflowService,workflowId:string)=>{const approval=service.requestActivation(workflowId,undefined,"test");service.decideApproval(approval.id,"approved","test");service.executeApproval(approval.id,"test");};
...
describe("trusted workflow evaluation and real canary",()=>{
  it("derives immutable receipts from actual bound runs and rejects tampering",()=>{
    const {store,service}=make(); const session=store.createSession(); const workflow=service.teach(session.id,spec,"message:1"); activate(service,workflow.id);
  ...
  it("uses stable buckets, binds real runs, aggregates outcomes, and automatically rolls back",()=>{
    const {store,service}=make(); const session=store.createSession(); const workflow=service.teach(session.id,spec,"message:1"); activate(service,workflow.id);
    const candidate=service.revise(workflow.id,{steps:[...spec.steps,{stepId:"build",instruction:"Build",required:true}]},"user_correction",["message:2"],"add build");
    trustedGates(store,service,session.id,workflow.id,workflow.revision!.id,candidate.id); const request=service.requestPromotion(workflow.id,candidate.id,25,0.02,"test");service.decideApproval(request.id,"approved","test");const promotion=service.executeApproval(request.id,"test").result as {id:string};
    ...
    expect(store.db.prepare("SELECT status FROM workflow_promotions WHERE id=?").get(promotion.id)).toEqual({status:"rolled_back"});
