...
describe("workflow learning HTTP API", () => {
  it("teaches, governs, records feedback, sets learning policy and forgets workflows", async () => {
    ...
    const taught = await app.inject({ method: "POST", url: `/api/sessions/${session.id}/workflows/teach`, payload: { spec, sourceId: "message:1" } });
    expect(taught.statusCode).toBe(200); const workflow = taught.json();
    expect(workflow.status).toBe("candidate");
    expect((await app.inject({ method: "POST", url: `/api/workflows/${workflow.id}/activate`, payload: {} })).statusCode).toBe(200);
