...
function RunDetails({ run }: { run: TaskRun }) {
  return <div className="run-details">
    ...
    <section className="run-summary"><div className="phase-line"><span className={`phase-badge ${run.status}`}>{run.status}</span><span>{run.phase}</span><span>attempt {run.attempt}</span></div><p>{run.goal}</p><div className="run-metrics"><span>{run.transcriptCount} messages</span><span>{run.usage.totalTokens.toLocaleString()} tokens</span><span>{run.usage.input.toLocaleString()} in / {run.usage.output.toLocaleString()} out</span>{run.budget && <span>{run.budget.tier}{run.budget.softTokens && run.budget.softTokens < run.budget.maxTokens ? ` · ${run.budget.softTokens.toLocaleString()} checkpoint` : ""} · {run.budget.maxContinuations} rounds max · {run.budget.maxTokens.toLocaleString()} tokens max</span>}</div>{run.blockedReason && <div className="blocked-note">{run.blockedReason}</div>}</section>
