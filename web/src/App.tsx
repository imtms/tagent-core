  ...
  return <div className="app-shell">
    ...
    <aside className={`run-panel ${rightOpen ? "mobile-open" : ""}`}>
      ...
      {!run ? <div className="panel-empty"><Play size={20} /><p>No active task</p></div> : <>
        <section className="run-summary"><div className="phase-line"><span className={`phase-badge ${run.status}`}>{run.status}</span><span>{run.phase}</span></div><p>{run.goal}</p>{run.blockedReason && <div className="blocked-note">{run.blockedReason}</div>}</section>
