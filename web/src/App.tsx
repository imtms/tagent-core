...
function RunDetails({ run, onRefresh }: { run: TaskRun; onRefresh?: () => Promise<void> }) {
  return <div className="run-details">
    ...
    {run.supervision.latestContextManifest && <section className="panel-section"><div className="section-title"><span>Context manifest</span><small>{run.supervision.latestContextManifest.source} · attempt {run.supervision.latestContextManifest.attempt}</small></div><div className="checkpoint-card"><strong>{run.supervision.latestContextManifest.items.filter((item) => item.selected).length} selected · {run.supervision.latestContextManifest.items.filter((item) => !item.selected).length} omitted</strong><span>{run.supervision.latestContextManifest.items.filter((item) => item.selected).reduce((sum, item) => sum + item.estimatedTokens, 0).toLocaleString()} estimated tokens</span><span>hash {run.supervision.latestContextManifest.manifestHash.slice(0, 12)}</span>{run.supervision.latestContextManifest.items.filter((item) => !item.selected).slice(0, 3).map((item) => <small key={`${item.kind}:${item.sourceId}`}>{item.kind}: {item.reason}</small>)}</div></section>}
