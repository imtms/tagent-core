...
The Host commits `current` only after the target Generation reports ready with a writer fence strictly higher than the prior Generation. A target that fails readiness or reports a non-advancing fence is replaced by the previous release, whose fence must advance again. Automatic binary rollback is allowed only while the state contract is unchanged. State evolution first advanced the manifest contract to r2 and now advances it to `tagent-core/state-0.8-r3`; the first r3 migration requires a full Host/service restart, and the Host thereafter rejects automatic rollback to r1/r2 binaries.

Host close tracks every activation Promise, marks the stop boundary before awaiting sessions, joins activation, rechecks after persisted phase transitions, and terminates any candidate that crossed close before returning. Generation instance-lock recovery markers carry the same process identity shape as the lock: live, remote, or unverifiable owners remain fail-closed, while a marker is reclaimed only after repeated checks prove its owner dead or PID-reused. Acquisition then revalidates its own lock before exposing writer authority.
...
`tests/core-service-workspace-package.test.ts` enforces the Host import boundary, private Generation entry, explicit managed-process exit, and the single Host system entrypoint. `tests/core-host.test.ts` covers strict/oversized IPC, backpressure, early parent disconnect, closed activation shape, stale drain identity, durable activation serialization, normal and same-release replacement, stable Generation working directories, monotonic writer fences, exact/conflicting replay, terminal-result persistence failure, forced drain, candidate readiness and commit-state rollback, deterministic Host crash points, close while starting-state persistence is paused, startup-child reclamation, direct development startup, and durable crash budgets. `tests/core-instance-lock.test.ts` proves dead/PID-reused owners can be recovered while live, remote, and unknown owners remain excluded, including an abandoned recovery marker. `tests/core-lifecycle.test.ts` verifies handoff preparation occurs after quiescence and before writer release.
...
The original phase added no application table. Revision 2 added only the append-only schema journal; revision 3 adds semantic-control provenance and advances release manifests to `tagent-core/state-0.8-r3`. `hostProtocolVersion: 2` and the unexported `node_modules/@tagent/core-service/dist/generation-entry.js` child entry remain unchanged by revision 3.
