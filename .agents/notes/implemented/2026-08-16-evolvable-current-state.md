...
Revision 3 adds semantic-control provenance, immutable plan/evidence records, accepted uncertainty, Context Manifest linkage, and Transcript FTS through the same preflighted migration runner. The state protocol is now `tagent-core/state-0.8-r3`. Release verification and Host activation reject r1/r2 manifests after migration, so the first r3 deployment requires backup and a full Host/service restart; subsequent compatible r3 Generation activation retains automatic rollback. Returning to r2 requires restoring the complete pre-r3 SQLite/WAL/SHM recovery set. Tests cover r1/r2 data preservation, repeat-open idempotence, append-only enforcement, failed-migration rollback, drift, newer revision rejection, manifest incompatibility, and readiness reading the real SQLite revision.
