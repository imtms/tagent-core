...
- `tests/operator-inbox-context-api.test.ts` proves that reusing one idempotency key against a different Inbox item conflicts for update, decision, merge, and delete.
- `tests/web-api.test.ts` proves complete Session/Inbox/Skill catalog/Skill revision cursor traversal and position-ordered queued Inbox projection.
