...
- Goal revision requests, evidence, decision history, and lifecycle controls now stay in one disclosure-led Goal workspace. Memory correction, dispute resolution, reactivation, receipt-ID recovery, export, and lifecycle filtering stay in the same catalog/detail/operations hierarchy instead of forming a second dashboard or card wall.
