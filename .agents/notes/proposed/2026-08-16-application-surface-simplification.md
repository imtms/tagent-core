...
- Goal revision requests, evidence, decision history, lifecycle controls, and user-visible operation request IDs now stay in one Goal workspace organized by stable Overview, Roadmap, Activity, and Controls views. Memory correction, dispute resolution, reactivation, receipt-ID recovery, export, lifecycle filtering, Recall, Core projection, operations, and explicit Record/Topic catalog layers stay in one stable four-view hierarchy instead of forming a duplicated text wall, accordion chain, second dashboard, or card wall. Surface simplification preserves complete operation reachability and changes only the information layer shown at one time.
