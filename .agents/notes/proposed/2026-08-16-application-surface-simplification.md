...
- Goal revision requests, evidence, decision history, lifecycle controls, user-visible operation request IDs, and pre-Run Supervisor queue state now stay in one Goal workspace organized by stable Overview, Roadmap, Activity, and Controls views. Memory correction, dispute resolution, reactivation, receipt-ID recovery, export, lifecycle filtering, Recall, Core projection, operations, and explicit Record/Topic catalog layers stay in one stable four-view hierarchy; Record and Topic detail add stable Overview, Metadata, and Controls views instead of another accordion chain. Surface simplification avoids duplicated text walls, second dashboards, and card walls while preserving complete operation reachability and changing only the information layer shown at one time.
