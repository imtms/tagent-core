...
| ABI SDK | `@tagent/abi@0.8.0` |
| Typed client SDK | `@tagent/core-client@0.8.0` |
...
| --- | --- | --- |
| Core `0.8.0`, current schema, all required profile summaries/details available | ABI + Core Client `0.8.0` | Supported |
| Core `0.8.0`, current schema | hand-written DTOs or direct Console/Admin calls | Unsupported |
| Any different Core release tuple | `0.8.0` SDKs | Unsupported unless Gateway separately proves and accepts it |
| Core `0.8.0`, required profile partially available/unavailable to the actual service principal | any | Unsupported for that profile until scopes/resource grants are corrected |
| Future profile minor with additive strict response fields | `0.8.0` strict decoder | Unsupported until a matching SDK is deployed |
| Future profile major | any `1.x` client | Unsupported without a coordinated major rollout and matching client |

Compatibility is negotiated per feature. A healthy Core or compatible base `/api/v1/capabilities` response does not enable a missing full-feature profile. Gateway must validate the registry and the selected profile detail using the same authenticated service principal that will call its endpoints.
...
```text
tagent-abi-0.8.0.tgz
tagent-abi-0.8.0.tgz.sha256
tagent-core-client-0.8.0.tgz
tagent-core-client-0.8.0.tgz.sha256
```
