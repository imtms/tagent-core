...
The Goal reading surface keeps the definition, status, progress and concrete next action above four stable views. Overview contains completion criteria and boundaries; Roadmap contains revision, approval and TaskRun launch/open actions; Activity contains linked Runs, evidence and decisions; Controls contains three explicitly named groups: Lifecycle, Revision request and Operation recovery. The current next action selects the most relevant initial view, but all four remain one click away. Raw receipt payload and outcome stay in a nested disclosure after the durable operation identity. This hierarchy reduces the visible text wall without removing Goal operations or depending on a vertical chain of top-level accordions.

During Roadmap review, every criterion mapping remains visible beside the item it authorizes. After approval, each item keeps its title, outcome, execution state and Start, Retry or Open action in the scan layer, while its criterion mappings and verification instruction share one explicit disclosure. Completion criteria, Roadmap items and Activity history each use one outer hairline ledger with internal row separators; simplification comes from stable views and local disclosure, not disconnected borderless rows or missing audit data.
