...
The first-party Console separates four user tasks into stable Memory-center views: Catalog filters the loaded Hot/Warm Record or Cold Topic directory, Recall performs semantic retrieval across all tiers, Core edits the stable injection snapshot, and Operations owns Reindex, receipt-ID restore, and job history. Record/Topic detail returns to the Catalog or Recall view that opened it and separates Overview, Metadata, and Controls into stable views rather than top-level accordions. Inactive Records and deleted Topics open Controls first so governance or recovery is immediately discoverable. Both catalog layers remain searchable, pageable and governable; lifecycle controls, provenance, Cold storage metadata, Recall diagnostics, Core Memory projection, jobs, export and restore remain reachable without stacking equivalent text or hiding the complete workflow below a chain of disclosures.
