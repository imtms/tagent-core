...
The supported persistent profile uses one active Core Generation with PostgreSQL 17, `vector`, `pg_trgm`, and Local Cold storage. The stable Host does not open Memory storage. The in-memory backend is for tests/development. S3 is implemented as a Cold adapter but is not the primary release-gated deployment profile.

Persistent Memory accepts an absent PostgreSQL `memory` schema, the exact current identity `tagent-memory/0.8` version `2`, or the exact version-1 shape as a migration input. Initialization creates version 2 directly; version 1 is upgraded transactionally under the Memory advisory lock by adding and backfilling the immutable Topic creation key used for stable pagination. An unmarked, differently identified, newer, or unsupported schema is rejected.
...
Records support correction, supersession, stale/delete thresholds, feedback, reversible tombstones, and retention. Reconfirming a superseded single-valued fact or preference reruns conflict resolution, so its active replacement is superseded instead of leaving both values active. A whole-Topic tombstone retains its published Cold revision and pointer through the configured grace period, allowing an exact restore; deleting only part of a still-live Topic invalidates its Cold revision so restoration cannot reintroduce content that remains forgotten. Restore targets explicit record or Topic IDs. Maintenance removes expired objects only after the policy boundary permits it.

Reindex is durable and generation-based. A new embedding generation is staged, checkpointed, validated, and activated only after completion; readers continue using the active generation during the build. Every reindex vector upsert goes through the job repository, which verifies the current owner, lease token, fencing token, generation, scope, and unexpired lease in the same PostgreSQL statement that writes the vectors. Final cleanup, old-generation retirement, new-generation activation, job completion, and vector generation garbage collection form one repository transaction guarded by the same owner/token/fence/scope/generation and database-clock lease check. Capture-job renewal, completion, and failure settlement enforce the same unexpired ownership boundary, and the in-memory adapter applies the equivalent rule. A worker whose provider call outlives its lease cannot renew, settle, delete newer vectors, or mutate generation state after another worker reclaims it.
...
Core Memory is generated from high-value active records. A capture refreshes only scopes whose durable Core-eligible records actually changed, so empty, failed, filtered, and idempotent retry jobs do not trigger a 50k-record rescan. Maintenance and ordinary approve, correct, forget, and restore synchronization preserve human-authored text while replacing or pruning provenance-marked generated lines. Only an explicit Generate action intentionally rebuilds the complete snapshot and clears that preserved manual layer.
...
The first-party Console separates four user tasks into stable Memory-center views: Catalog filters the loaded Hot/Warm Record or Cold Topic directory, Recall performs semantic retrieval across all tiers, Core edits the stable injection snapshot, and Operations owns Reindex, Record/Topic ID restore, and job history. Record/Topic detail returns to the Catalog or Recall view that opened it and separates Overview, Metadata, and Controls into stable views rather than top-level accordions. Inactive Records and deleted Topics open Controls first so governance or recovery is immediately discoverable. Both catalog layers remain searchable, pageable and governable; lifecycle controls, provenance, Cold storage metadata, Recall diagnostics, Core Memory projection, jobs, export and restore remain reachable without stacking equivalent text or hiding the complete workflow below a chain of disclosures.

Record and Topic detail keeps kind, tier, lifecycle state, scores, revision, token count and storage availability in one quiet mono identity line above the title. Metadata uses one bounded ledger with internal hairline rows for identifiers, provenance, semantic identity, lifecycle, validity, relationships and Cold storage. Controls preserve every state-appropriate action: candidate approval/rejection, dispute resolution, stale reactivation, correction, feedback, Forget and recovery. Deleted Records expose Restore without leaving expired correction or feedback actions beside it.
