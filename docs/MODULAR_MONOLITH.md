...
Core application construction accepts one named options object. Internally, Admission, Execution, Governance, Workspace Goals, Memory, and Skills stay grouped; the flat HTTP-facing application ABI is bound once from an explicit method whitelist with missing-method and collision checks. `CoreExternalActionApprovalApplication` owns the approval-request/Attempt-transition transaction, while `CoreRunFinalizationApplication` owns post-Attempt Workspace Goal reconciliation and next-Inbox dispatch. Both remain in-process over the same writer-fenced persistence ports; they add neither a network boundary nor a second mutation authority. Web uses CoreClient for ABI/SSE transport and keeps workspace loading/preferences, Transcript projection, ACK scheduling, and presentation components outside the root component.
