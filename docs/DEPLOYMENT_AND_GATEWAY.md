...
Core 0.8 keeps schema ID `tagent-core/0.8` and reports numeric revision `3`. It creates an empty database through the ordered migration runner and upgrades the exact legacy revision-1, revision-2, and pre-`user_version` 0.8 shapes without losing rows. The append-only migration journal, checksums, revision, and complete schema shape are verified on every open.

Before the first revision-3 start, stop all writers and back up SQLite together with WAL/SHM files. A marker mismatch, newer revision, journal mismatch, or schema drift is a deployment failure; restore a verified backup instead of editing metadata or copying rows.
...
3. back up and validate the existing exact revision-1/revision-2 database, or provision an empty SQLite path, plus matching optional Memory stores;
...
6. require base capabilities to report schema version `3`, current catalogs, Operator endpoints, ready Approval, receipt recovery, retention, limits, and `operator.read.v1`;
...
sudo /path/to/deploy-release.sh \
  /path/to/tagent-core-v0.8.32-linux-x64-node24-abi137.tar.gz \
  /opt/tagent-core
```
...
Do not restart the hosting service from the generic Bash tool. Core rejects common `systemctl`, `sudo systemctl`, `service`, init-script and nested-shell stop/restart forms with guidance to use `core_generation_activate`, because an in-band service restart can terminate the active tool before its result and remaining stages are durably settled. This protection is deliberately scoped to the hosting `tagent-core` service; it does not claim to sandbox general process administration.
...
A rollback build may reuse the migrated database only when its release manifest declares `tagent-core/state-0.8-r3` and it accepts `tagent-core/0.8` revision 3 plus the current ABI/profile tuple. The first r3 deployment requires a full Host/service restart; after migration the Host rejects old r1/r2 release manifests. To return to r2 or r1, stop the service and restore the matching pre-upgrade SQLite/WAL/SHM recovery set. Never point an incompatible binary at the current database or overwrite live database files.
