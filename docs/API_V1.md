...
Synchronous resource mutations require `Idempotency-Key` and an `If-Match: "rN"` ETag. The first canonical request identity—including any item identifier carried in the route—complete public result projection, and ETag source are retained for exact replay. Core resolves that receipt before mutable deployment validation, live readback, Router analysis, or filesystem staging; a changed target or payload returns `409 idempotency.conflict`, and a stale revision returns `409 concurrency.conflict` with the current ETag. Asynchronous or externally observable mutations require `Idempotency-Key` and return a durable operation receipt. Recover those receipts at `/api/v1/operator/operations/:requestId` or `/api/v1/admin/operations/:requestId`; `outcome_unknown` requires reconciliation and is never permission for automatic replay.

Profile list cursors freeze membership with immutable creation order keys while returning current public projections. Updating an unread item cannot move it past the cursor. Memory pages query storage with `limit + 1`; the 200-item HTTP maximum is a per-page bound, not a collection cap. When the first-party Console needs a complete collection, it follows `pageInfo.nextCursor` through every Session, Inbox, Skill-catalog, and Skill-revision page. Its queued Inbox projection then sorts by `position`, `createdAt`, and `id`, independently of the transport's historical creation order.

Session Inbox update, reorder, decision, merge, and delete operations exist only through the declared `operator.session-inbox.v1` profile endpoints. Item mutations bind the route's `itemId` into their canonical idempotency identity and always use the profile's receipt, resource scope, and collection-revision checks; Core does not expose a parallel unreceipted application mutation surface.
...
After user editing and approval, `task-runs` starts an approved Roadmap item through normal admission. A successfully completed item deterministically admits the next untouched approved item in Roadmap order; failed, cancelled, blocked, interrupted, queued or already-running work stops automatic progression and is never implicitly retried. Ordinary user-started TaskRuns in the Workspace automatically receive the unique active Goal definition as immutable direction. Goal detail and summary `nextAction` may include `taskRunId` when the user must open a specific Run. Each Roadmap progress entry includes its Goal-level `status`, underlying nullable `runStatus`, `runId` and `retryable`: `failed`/`cancelled` work can be retried, while `blocked`/`interrupted` work must be resumed or resolved in the original Run. `/plans`, `/run-links` and `/evidence` are removed and return 404.
...
Supported commands are `task_run.steer`, `task_run.follow_up`, `task_run.cancel`, `task_run.resume`, `task_run.compact`, `task_run.submit_user_input`, `task_run.resolve_approval`, and `task_run.accept_uncertainty`. `steer` and `follow_up` return after the fenced control intent is durable; Runtime delivery continues asynchronously. `TaskRun.pendingInteractions` is the typed source for pending Approval and User Input UI. Gateway needs only `runs:read` and `runs:control`.

`task_run.resume` advances ordinary resumable Runs immediately. For a Run whose immutable policy or approval history includes an external action, the same command succeeds by creating a pending `execute_external_action` interaction bound to `currentAttempt + 1`; it normalizes interrupted or timeout-failed state to `blocked` but does not advance the Attempt or launch Runtime. The client must refresh the TaskRun, render that approval, and use `task_run.resolve_approval`. Only approval of that exact bound request advances the Run. This also applies after restart, a resumable idle/hard timeout, or dynamic discovery of an explicit external tool; automatic crash-recovery Continuations never bypass it. If context preparation fails after a resume has created the next Attempt, Core persists a `run.failed` event and terminalizes that Attempt instead of leaving it running without Runtime ownership.

`task_run.accept_uncertainty` is an authorized `runs:control` operation, not an Agent tool or effect approval. It accepts one currently `unsupported` or `blocked` Run-local `ac-*` criterion with principal-derived actor identity, rationale, scope, optional evidence references, and optional expiry. The command claim follows the standard receipt-first recovery model; the accepted-uncertainty mutation and any same-Candidate re-adjudication commit in one writer-fenced Unit of Work before the command receipt is settled. An interruption between those boundaries leaves the command receipt recoverable as `outcome_unknown`, while the decision identity prevents the semantic mutation from being duplicated.
