...
- keeps the Core system prompt stable for one Workspace/project-rule snapshot and appends current TaskRun, policy, Goal, and recalled Memory as an ephemeral final user message before every provider request;
...
The adapter imports that bounded transcript into an in-memory Harness Session. It projects historical tool output and TaskRun receipts before provider calls, supports explicit compaction, performs threshold compaction after successful turns, and performs one compaction-and-retry cycle after a provider context-overflow response. The fixed system prompt contains only Core instructions, the Workspace path, and the current project-rule snapshot. Immediately before every provider request, Core reloads the current TaskRun and appends one ephemeral final user message containing runtime state, execution policy, Workspace Goal direction, and recalled Memory. That message enters the real provider payload and durable request envelope but is never appended to the Harness Session or durable transcript. Context assembly reserves its token budget before selecting history.
