...
Before an Attempt starts, Core persists an immutable Context Manifest describing selected Session messages, transcript material, TaskRun contract, prompt, Core Memory, dynamic Memory records, Cold Topics, omissions, token estimates, and projection hashes. Every selected item carries a `projectedContentHash` and durable `sourceRevision`; the system entry hashes Core's `{systemPrompt, attemptContext, liveContext}` projection and does not claim to be the provider-payload hash. Selected Memory projections are atomically copied into a private append-only evidence-source ledger with the same revision and hash, so later Supervisor quotes resolve the exact bytes used by that Run even if live Memory changes. Raw bytes are not exposed through the ordinary Context Manifest API. Runtime input is assembled from these bounded projections, not by letting the provider read the database.
...
Compaction is session-local; the durable Transcript and Context Manifest remain Core-owned. Transcript rows are append-only so an exact-sequence Evidence Quote cannot be invalidated by retention or editing; long-term compaction never deletes quoted source rows. Because a summary can omit an exact path, identifier, failure code, or middle-of-output fact, `history_search` exposes bounded, case-sensitive literal search and Unicode full-text term search over only the current TaskRun's durable Transcript. Search supports Attempt, durable role, rendered kind, and durable write-time filters, uses an exclusive `beforeSeq` cursor, returns `nextBeforeSeq` for stable backward pagination, and excludes the tool call's own or any future Transcript entry. `action=get` retrieves one exact earlier sequence for quote verification. Public Transcript paging exposes the same filters while preserving tool-call/result pairing. Every mode remains same-Run only and cannot enable regex or an arbitrary result limit.
...
Immediately before each Pi provider call, after provider dialect conversion and every model-visible transform are complete, Runtime persists an Attempt request envelope. The exact provider request body is the single replay truth; the envelope adds only Attempt/request identity, model metadata, schema version, timestamp, Context Manifest identity, and hashes rather than maintaining duplicate prompt, message, tool, Skill, or reasoning projections that could drift from the transmitted body. SQLite verifies that the referenced manifest belongs to the same Run and Attempt, records the manifest-to-envelope usage link, then reads the row back and verifies both the provider-payload hash and complete envelope hash. Any missing, changed, cross-Run, cross-Attempt, or corrupt read aborts before network dispatch. The manifest exposes only envelope IDs; it never copies provider payload bytes. `request.envelope.persisted` exposes hashes and identity only, never authorization material.
...
Router, Supervisor, and Workspace Goal Roadmap generation use separately bounded model, context, timeout, and output settings. Router input is projected head/tail inside its configured window and deterministic fallback emits at most 12 objectives and 24 criteria. Routing crosses Admission as one explicit `{analysis, usage, provenance}` envelope: source/projection hashes, model attempt/success, bounded failure detail, abstention, and any usage observed before local parse rejection survive object cloning and Inbox edits. Usage is charged to a Run only when the routed action actually targets or creates that Run; an edited queued item keeps provenance without charging an unrelated active Run. The effective routing confidence is the lower of semantic routing and execution-policy confidence; active-Run control below `0.85` and a `new_task` or `parallel_task` below `0.7` remain durably queued as `needs_clarification` until an operator edits or explicitly starts them. The legacy `merge_candidate` value remains readable in durable/public ABI, but model routing cannot produce it without an operator-selected merge target. Credential resolution completes before the transport response-header timer starts, so a slow trusted credential provider is not misclassified as a provider timeout. Response-header timeout, streaming body idle timeout, and credential-resolution failure remain distinct diagnostics. Their default transport timeout is 5 seconds unless the owning component documents another value. Common unambiguous admission decisions and known runtime failures are classified locally; settled semantic review follows the call policy in [SUPERVISOR.md](SUPERVISOR.md).
...
Submission, steer, and follow-up content is limited to 200,000 characters. Context assembly enforces the effective model budget even for the latest turn by projecting historical text/tool arguments while retaining the complete durable source in SQLite or an Artifact. The current runtime prompt, stable Attempt context, live context, reserved output, and real contract wrapper are never silently truncated: if those fixed inputs alone exceed the model window, Core fails before provider dispatch with an explicit budget diagnostic.
...
`@tagent/workspace-local` provides contained `ls`, `read`, create-only `write`, snapshot-CAS replacement, snapshot-bound `edit`, atomic multi-file `patch`, `bash`, and bounded same-Run Transcript recall plus TaskRun control integration through the Execution-owned `RuntimeTool` ABI. `write` rejects an existing target unless the caller supplies the current snapshot identity; it therefore cannot silently replace a file observed earlier or never read. Concrete tools are grouped into independent Tool Providers. `ToolRegistry` rejects duplicate names and freezes an Attempt-local catalog snapshot; `ToolExecutionPipeline` is the non-bypassable path for current-Attempt fencing, effect approval, external-action and Workspace Goal guards, tool-attempt records, operation receipts, idempotent replay, check invalidation, and single settlement. `runtime-pi` converts the already wrapped catalog to `AgentHarnessTool` at the adapter edge.
...
`bash` delegates process creation to `SubprocessPort`. Before dispatch, its stage-aware classifier assigns deterministic effects. Proven contained file observations remain approval-free; network access, process control, external mutation, paths outside the workspace, parse uncertainty, and workspace code execution require explicit current-Attempt effect approval. Test runners, linters, build scripts, and plugin-capable verification are recorded as `code_execution`, not as read-only observation, because repository configuration and plugins are executable. Dangerous observation options such as ripgrep preprocessors, Git external diff/textconv/output, sed execution or file-writing programs, and find actions fail out of the observation class. Unknown commands fail closed instead of inheriting a low-risk Run classification. The local adapter builds a new child environment by removing all `TAGENT_*` variables and credential-shaped names (`KEY`, `TOKEN`, `SECRET`, `PASSWORD`, `CREDENTIAL`, authorization, and cookies), then applies only explicit trusted overrides. POSIX children run in their own process group; abort, timeout, Attempt finalization, host disposal, and a main child that exits while descendants survive terminate the whole group with TERM-to-KILL escalation. Process-tree cleanup has a finite total deadline and fails explicitly rather than retaining runtime ownership forever. This is lifecycle and secret containment, not an OS sandbox.

Large output may spill to a durable Artifact with a bounded preview. The temporary capture descriptor and file are closed and removed on success and on every execution, synchronization, or Artifact-persistence failure. Read-only Operations are claimed before dispatch; a timeout, cooperative abort, or failure therefore settles a diagnostic `observation_failed` receipt with an explicit read-only effect, but never invalidates checks or qualifies as completion evidence. Successful Bash results carry a Core operation ID and digest that can bind a TaskRun check; a model-authored success claim cannot replace that receipt. Mutation-capable operations invalidate earlier trusted checks even when the mutation reports failure because partial effects cannot be assumed absent. Failed or timed-out identical Bash commands are fenced from blind re-execution, and composite commands receive split-stage guidance. The stage-aware command classifier does not treat mutation words inside quoted/read-only arguments as effects, while the catastrophic-command matcher recognizes common equivalent flag and wrapper forms. Both path containment and command policy remain guardrails, not an OS sandbox. Operation receipts and approval policy remain Core-owned even when Pi initiated the tool call.
