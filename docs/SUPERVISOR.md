...
The review receives bounded actual operation payloads, results and effects, including Bash command, exit code, output, digest, Artifact and completion time. A `succeeded` status alone is not semantic proof: the LLM must compare the receipt content with the criterion and candidate claim. Evidence references may point only to trusted checks, supplied operations, substantive artifacts, exact Transcript entries, or selected Memory records/revisions. The final provider projection selects operation receipts explicitly linked by required checks, plan completion, or prior Gate coverage before filling its bounded window with recent operations; linked early sources therefore remain both quotable and allowed. Candidate prose is not independent proof of its own claims, and invented references fail local validation.
