...
The catalog shows one storage layer at a time through an explicit Cards / Topics selector. Both layers remain one action away, but equivalent Record and Topic text must not be stacked into a duplicated mobile reading wall. Recall results belong to the submitted cue and kind; changing the cue or filters invalidates the old result instead of relabeling stale evidence.
