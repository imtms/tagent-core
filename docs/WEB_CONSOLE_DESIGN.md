...
Requesting changes must target the active Definition or Roadmap and retain an operator reason; it must not require leaving the Goal workspace. Durable Goal operation receipts remain recoverable by request ID in Controls, but the entire recovery surface stays behind one closed advanced disclosure. Receipt lookup exposes readable state and operation type first; request IDs, payload hashes, payload, result, and error stay together behind a nested technical receipt disclosure. Decision history translates Web Console and operator actor IDs into `You` instead of presenting implementation identities as people. The interface never encourages repeating an interrupted operation whose outcome is uncertain.
...
The Memory header exposes only non-zero lifecycle counts and stays silent while those counts load; scope IDs and backend adapter names never fill that gap. Tabs omit zero counts and em-dash placeholders. Kind and status filters share the search toolbar; correction, confirmation, helpful/wrong feedback, disputed resolution, reactivation, and Forget remain in detail Controls. Forget must expose immediate Undo for the returned grace period; recovery by Record/Topic ID remains in Memory operations because forgotten records are intentionally absent from the catalog. Add-memory copy says only that the value is scoped to the current Workspace, never its storage key. JSON export is a quiet toolbar action.
...
Record provenance, semantic identity, lifecycle, validity, storage identity, and Topic relationships belong in one two-column hairline ledger inside the Metadata view rather than separate cards or stacked paragraphs. Scope and storage identity appear once inside that view, not again in its heading. Recall routing, score construction, embedding degradation, policy transforms, and candidate outcomes belong in one diagnostics disclosure; aggregate trace counts never float below results without inspectable diagnostics. No-result guidance points to the Catalog view rather than claiming the catalog is below the results. Metadata remains one click away while complete operator data stays available. Generated Record or Topic titles that merely prefix the body or description with a Memory kind are treated as labels and must not repeat the same content in catalog, Recall, or detail views. A Record summary that repeats either the visible title or body is likewise omitted.
...
Supervisor queue rows keep one request summary, defer state, and immediate actions visible. When the machine summary differs from the submitted request, the original request and distinct, non-empty acceptance criteria remain inspectable inside `Task details` instead of becoming duplicate primary paragraphs. Intent, relation, priority, confidence, and a distinct routing reason move one level deeper under `Routing details`; target Run IDs disappear because they do not help operate the queue. If routing is the only available detail it uses the single outer disclosure rather than creating a disclosure inside an otherwise empty disclosure. Generated URLs and continuous text wrap inside the row at narrow widths.
