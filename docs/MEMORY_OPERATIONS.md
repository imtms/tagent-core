...
## 0.1.5 durable operations

Memory reindex is represented by durable `memory.reindex_jobs` rows. Workers claim with a lease token and monotonic fencing token, checkpoint after each embedding batch, stage generation metadata, and switch the completed generation to `active` before old-generation cleanup. Operators can queue and inspect jobs from Memory Center or `/api/memory/reindex` and `/api/memory/reindex/jobs`.

Readiness performs actual embedding and extractor probes and reports persistent heartbeat, backlog oldest age, dead letters, P95 capture/consolidation latency, error rates, embedding pending/completeness, active generation and the last degraded event.

Topic forget now creates a tombstone and retains immutable Cold revisions during the configured grace period. Restore accepts `topicIds`; maintenance deletes revision objects only after expiry.

Core Memory is a revisioned Markdown projection synthesized from high-value active records. It is regenerated by maintenance when evidence changes, can be edited in Memory Center, and receives a deterministic prompt budget before dynamic recall cards.
