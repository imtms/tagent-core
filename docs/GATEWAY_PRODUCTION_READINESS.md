...
store.close();
process.stdout.write(JSON.stringify({ schemaVersion, objects, hasGoalLinkMode: goalRunLinkColumns.includes('link_mode'), hasContinuationNotBefore: continuationColumns.includes('not_before') }));
NODE
```
