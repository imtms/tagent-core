...
Use a dedicated workspace without SSH keys, provider credentials, cloud config, production secrets, release roots, or unrelated files. The supported activation path is the receipt-backed `core_generation_activate` tool. Core deterministically rejects common Bash forms that stop or restart its own `tagent-core` service, including wrapper and nested-shell variants, before external-action approval or subprocess dispatch; commands administering unrelated services and lifecycle text used only as data remain available. This narrow self-lifecycle guard is not an OS sandbox: if the Generation's OS identity can mutate the release root or reach an unrecognized process-control path, permitted process tools may still affect the host. Apply OS/container controls for filesystem, network, process, and resource isolation when stronger containment is required.
