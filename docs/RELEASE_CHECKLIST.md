...
- [ ] `core_schema.schema_id` is `tagent-core/0.8`, `Store.getSchemaVersion()` and `PRAGMA user_version` are `3`, the migration journal/checksums are exact and append-only, and `sqlite_master` drift validation fails closed.
- [ ] The Core manifest declares `tagent-core/state-0.8-r3`; r1/r2 manifests are rejected after migration, and the pre-upgrade SQLite/WAL/SHM recovery set has been tested.
