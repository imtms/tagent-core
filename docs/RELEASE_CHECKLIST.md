...
- [ ] The candidate migrated v30 → v31 → v32 → v33 → v34 → v35 → v36 → v37 → v38 → v39 → v40 → v41 → v42 and reopened idempotently.
- [ ] `schema_meta.version` is 42; trusted-evidence and Goal-execution drift checks pass; complete v39 receipt/ACK shapes, the v40 Submission audit shape, exact v41 Operator Read indexes, and the v42 Inbox execution-policy column pass fail-closed validation; `migration_issues` has zero open rows.
